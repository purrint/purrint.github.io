...
3. **Text mode**  
   ...
   - A second toggle row picks one of three sub-modes, described by `TEXT_STYLES` in `PurrintApp.tsx` (Markdown stylesheet variant + font classes + size), which drives both the editor and the render:  
     - **Small** — 16px IBM VGA bitmap (`.markdown-bitmap`). Heading sizes stay integer multiples of 16px and weight comes from a 1px text-shadow, because the bitmap smears at fractional scales and under synthetic bold.  
     - **Medium** — 24px Roboto with Roboto Slab headings (`.markdown-proportional`), for readable body text.  
     - **Large** — the same faces turned sideways (`.markdown-banner`, `writing-mode: vertical-lr`), so lines stack across the paper's width and run down its length. `fitBanner` then solves for the font size that makes those lines exactly span 384px, which is why one line prints twice as big as two. `MAX_BANNER_LENGTH` caps how far down the roll that can run.  
